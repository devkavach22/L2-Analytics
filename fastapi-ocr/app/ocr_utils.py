...
# # # FIX: Default to 'ocrrecords' (lowercase plural) to match Mongoose
# # OCR_COLLECTION = os.getenv("OCR_COLLECTION", "ocrrecords") 

# # if not MONGO_URL:
# #     raise ValueError("MONGO_URL is not set in .env file or environment variables.")
# # if not MONGO_DB_NAME:
# #     raise ValueError("MONGO_DB_NAME is not set in .env file or environment variables.")
...
# # def read_simple_text(file_bytes: bytes) -> str:
# #     """Read text from simple text files with robust encoding detection."""
# #     encodings = ['utf-8', 'utf-16', 'latin-1', 'cp1252', 'ascii']
# #     for encoding in encodings:
# #         try:
# #             return file_bytes.decode(encoding).strip()
...
# # # ----------------------------
# # def extract_pdf(pdf_bytes: bytes) -> str:
# #     """Extract text from PDF (embedded + OCR fallback)."""
# #     text_pages = []
# #     pdf = fitz.open(stream=pdf_bytes, filetype="pdf")

# #     for page_index, page in enumerate(pdf):
# #         embedded_text = page.get_text().strip()
# #         if len(embedded_text) < 20:  # likely scanned page
# #             pix = page.get_pixmap(dpi=300)
# #             img_bytes = pix.tobytes("png")
# #             ocr_text = ocr_image(img_bytes)
# #             text_pages.append(f"--- PAGE {page_index+1} ---{ocr_text}")
# #         else:
# #             text_pages.append(f"--- PAGE {page_index+1} ---{embedded_text}")
...
# # def extract_sql(sql_bytes: bytes) -> str:
# #     """Extract text from SQL files (plain text)."""
# #     return read_simple_text(sql_bytes)
...
# #     if ext == ".sql":
# #         return extract_sql(file_bytes)
# #     if ext in [".txt", ".csv", ".json", ".md", ".log", ".xml", ".yaml", ".yml"]:
# #         return read_simple_text(file_bytes)

# #     # Fallback: OCR for unknown binary files
# #     try:
# #         return ocr_image(file_bytes)
# #     except:
# #         return read_simple_text(file_bytes)
...
# # # ----------------------------
# # def fetch_text_from_mongo(user_id: str) -> str:
# #     """Fetch all extractedText values for a user."""
# #     # Ensure matching 'userId' field name (standard Mongoose)
# #     documents = collection.find({"userId": user_id})
# #     merged_text = [doc.get("extractedText", "") for doc in documents if doc.get("extractedText")]
# #     return "\n".join(merged_text).strip()
...
# # def search_mongo_by_keyword(user_id: str, keyword: str) -> str:
# #     """
# #     Searches the existing collection for documents containing the keyword.
# #     """
# #     if not keyword:
# #         return ""
        
# #     print(f"DEBUG: Searching MongoDB ({OCR_COLLECTION}) for User {user_id} with keyword '{keyword}'")
...
# #     print(f"DEBUG: Found {len(documents)} matching docs.")
    
# #     results = []
# #     for doc in documents:
# #         filename = doc.get("originalFilename") or doc.get("fileName") or "Unknown File"
# #         text = doc.get("extractedText", "")
...
# #     # Handle latin-1 encoding issues
# #     safe_text = text.encode('latin-1', 'replace').decode('latin-1')
...
# # --- New Imports for YouTube Logic ---
# # Assumes utils.py and transcript_fetcher.py are in the same directory or python path
# try:
#     from utils import extract_video_id
#     from transcript_fetcher import TranscriptFetcher
# except ImportError:
#     print("Warning: YouTube modules (utils.py, transcript_fetcher.py) not found. YouTube features will be disabled.")
...
# # ----------------------------
# # EasyOCR Reader & YouTube Fetcher Initialization
# # ----------------------------
# READER = easyocr.Reader(['en'], gpu=False)

# # Initialize TranscriptFetcher if available
# yt_fetcher = TranscriptFetcher() if TranscriptFetcher else None
...
# def process_youtube_content_if_present(text: str) -> str:
#     """
#     Checks if the text contains a valid YouTube URL/ID. 
#     If yes, fetches and appends the transcript.
#     """
#     if not text or not extract_video_id or not yt_fetcher:
#         return text

#     # Check if the text content itself looks like a YouTube URL or ID
#     # We strip whitespace to ensure clean matching
#     clean_text = text.strip()
    
#     # Attempt to extract an ID from the text
#     video_id = extract_video_id(clean_text)
    
#     if video_id:
#         print(f"DEBUG: Detected YouTube Video ID {video_id}. Fetching transcript...")
#         try:
#             transcript = yt_fetcher.fetch_transcript(video_id)
#             if transcript:
#                 return f"--- YOUTUBE TRANSCRIPT (Video ID: {video_id}) ---\n{transcript}\n\n--- ORIGINAL LINK ---\n{clean_text}"
#         except Exception as e:
#             print(f"Error fetching transcript for {video_id}: {e}")
#             return f"{text}\n\n[System: Detected YouTube link but failed to fetch transcript: {str(e)}]"
...
#     # Post-processing: Check if the extracted text is a YouTube URL
#     # We only check this for text-based extractions (txt, sql, etc)
#     if extracted_text:
#         return process_youtube_content_if_present(extracted_text)
